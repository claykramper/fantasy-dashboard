...
def find_matchup(data, my_team):
    ...
    for game in data.get('schedule', []):
        if game.get('matchupPeriodId') != scoring_period:
            continue
        home = game.get('home', {})
        away = game.get('away', {})
        opponent_id = (
            away.get('teamId') if home.get('teamId') == my_team['id']
            else home.get('teamId') if away.get('teamId') == my_team['id']
            else None
        )
                ...
                if key in wanted and key not in seen:
                    ...
                    for stat in stats:
                        if (
                            ...
                            and stat.get('seasonId') == int(ESPN_SEASON)
                            and stat.get('scoringPeriodId') == period
...
def build_espn_matchup(nfl_games):
    ...
    return {
        ...
        'my_team': my,
        'opponent': opp,
        'difference': my['actual_total'] - opp['actual_total'],
    }
