...
# Used for Yahoo OAuth state.
#
# Set FLASK_SECRET_KEY in Render so sessions remain stable.
...
def yahoo_callback():
    ...
    # --------------------------------------------------------
    # Store tokens temporarily in the Flask session.
    #
    # We will replace this with persistent token handling
    # once Yahoo Fantasy API access is approved.
    # --------------------------------------------------------
    ...
    if refresh_token:
        session["yahoo_refresh_token"] = refresh_token
...
def group_games_into_slates(
    games
):

    """
    Group games that begin within 45 minutes
    of one another.
    """
