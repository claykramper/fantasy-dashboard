...
def yahoo_sync():
    ...
    Kept temporarily so the old LeagueSync sync script still works
    if needed. The dashboard now refreshes Yahoo automatically.
    """
    ...
    if not expected_token:
        return jsonify({
            'error': 'LEAGUESYNC_SYNC_TOKEN is not configured on the server.'
        }), 500
    ...
    if not isinstance(data, dict):
        return jsonify({
            'error': 'Expected a Yahoo league JSON object.'
        }), 400

    if str(data.get('platform', '')).lower() != 'yahoo':
        return jsonify({
            'error': 'Posted data is not a Yahoo LeagueSync league.'
        }), 400
    ...
    if not isinstance(roster, list) or not isinstance(matchup, dict):
        return jsonify({
            'error': 'Yahoo data is missing roster or current_matchup.'
        }), 400
